...
public class SearchAdapter extends BaseAdapter implements Filterable {
    ...
    public SearchAdapter (Context context, ArrayList<Items> itemsList){
...
//        accreditation.setText( itemsList.get( position ).getAccreditation() );
        ...
        type.setText( itemsList.get( position ).getSearch() );
            ...
            if(constraint != null && constraint.length()>0){
                ...
                for(int i=0; i<filterList.size();i++){
                    if(filterList.get(i).getBrand().toUpperCase().contains(constraint)){
                        ...
                        p.setSearch( "Brand Name" );
                        filters.add(p);
                    }
                    if (filterList.get( i ).getAccreditation().toUpperCase().contains(constraint)){
                        Items p = filterList.get(i);
                        String acc=p.getAccreditation();
                        p.setBrand( acc );
                        p.setSearch( "Accreditation" );
                        filters.add(p);
                    }
                }
